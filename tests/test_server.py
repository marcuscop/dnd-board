...
from dnd_board.application.action_service import append_roll_log_entry
from dnd_board.application.character_state_service import (
    reset_applies_to_rest,
...
from dnd_board.rules.classes.rogue.archetypes import RogueSubclassAbilityType, RogueSubclassAttackType, RogueSubclassRollActionType
...
def hit_point_grants(class_type: ClassType, *amounts: int) -> list[ProgressionGrantRecord]:
    return [ProgressionGrantRecord(
        source=ProgressionGrantSource(ProgressionChoiceId.HIT_POINT_INCREASE, class_type),
        grants=tuple(
            HitPointGrant(
                characterClass=class_type,
                minimumClassLevel=level,
                choice=HitPointChoiceOption.FIXED,
                amount=amount,
                hitDieResult=class_hit_die(class_type) // 2 + 1,
            )
...
def ability_score_grants(
    ...
    choice_id = {
        ClassType.FIGHTER: ProgressionChoiceId.FIGHTER_ABILITY_SCORE_IMPROVEMENT,
        ClassType.ROGUE: ProgressionChoiceId.ROGUE_ABILITY_SCORE_IMPROVEMENT,
        ClassType.WIZARD: ProgressionChoiceId.WIZARD_ABILITY_SCORE_IMPROVEMENT,
    }[class_type]
    return [ProgressionGrantRecord(
...
def feat_grants(
    ...
    return [ProgressionGrantRecord(
        source=ProgressionGrantSource(choice_id, class_type),
        grants=(FeatGrant(class_type, class_level, feat),),
    )]
...
def test_save_round_trips_suppression_ongoing_and_scheduled_effects(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(server, "SAVE_DIR", tmp_path)
    room = server.get_or_create_room("effect-save-test")
    ongoing = ActiveOngoingEffect(
        id=OngoingEffectId(123, EffectNodeId((2, 1))),
        sourceSheetId="player-1",
        targetSheetId="player-2",
        sourceLabel="Test Effect",
        effect=OngoingEffect(EffectDuration(EffectDurationType.UNTIL_LONG_REST)),
    )
    room.suppressed_conditions["player-2"] = [ConditionType.FRIGHTENED]
    room.ongoing_effects["player-2"] = [ongoing]
    scheduled = ActiveScheduledEffect(
        id=ScheduledEffectId(456, EffectNodeId((3,))),
        sourceSheetId="player-1",
        targetSheetId="player-2",
        sourceLabel="Delayed Effect",
    ...
    room.scheduled_effects["player-2"] = [scheduled]
    source = RollSource(SheetSectionType.ABILITIES, "fortifyingFeature", "apply")
    increase = ActiveMaxHitPointIncrease(5, source, "Fortifying Feature")
    reduction = ActiveMaxHitPointReduction(3, source, "Weakening Feature", RestType.LONG_REST)
    room.max_hit_point_increases["player-2"] = [increase]
    room.max_hit_point_reductions["player-2"] = [reduction]

    server.save_room_to_disk(room)

    loaded = load_room_save(server.save_path(room.id))
    assert loaded is not None
    assert loaded.suppressed_conditions == {
        "player-2": [ConditionType.FRIGHTENED]
    }
    assert loaded.ongoing_effects == {"player-2": [ongoing]}
    assert loaded.scheduled_effects == {"player-2": [scheduled]}
    assert loaded.max_hit_point_increases == {"player-2": [increase]}
    assert loaded.max_hit_point_reductions == {"player-2": [reduction]}
...
def test_sheet_roll_permissions_and_payload(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 12)
    client = TestClient(server.app)
    ...
    assert own_roll_body["label"] == "Longsword"
    ...
    assert own_roll_body["damageComponents"][0]["damageType"] == "slashing"
    assert forbidden.status_code == 403
...
def test_sheet_roll_logs_and_pending_rolls_are_room_visible_to_everyone(tmp_path, monkeypatch) -> None:
    longsword = AttackAction("longsword", "Longsword", AbilityType.STRENGTH, 1, DiceType.D8, damageType=DamageType.SLASHING)
    write_party_campaign(
        tmp_path,
        "shared-log-test",
...
def test_ad_hoc_dice_roll_is_logged_for_everyone(monkeypatch) -> None:
    client = TestClient(server.app)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: maximum)

    response = client.post("/api/rooms/ad-hoc-dice-test/dice?playerKey=player-1&dice=d8&count=3")
    invalid = client.post("/api/rooms/ad-hoc-dice-test/dice?playerKey=player-1&dice=d100&count=1")
    dm_state = client.get("/api/rooms/ad-hoc-dice-test/sheet?playerKey=dm").json()
    player_state = client.get("/api/rooms/ad-hoc-dice-test/sheet?playerKey=player-2").json()

    assert response.status_code == 200
    assert response.json()["roll"]["source"]["section"] == "diceRoller"
    assert response.json()["roll"]["label"] == "3d8"
    assert response.json()["roll"]["dice"] == [8, 8, 8]
    assert response.json()["roll"]["total"] == 24
    assert response.json()["logEntry"]["roll"]["id"] == response.json()["roll"]["id"]
    assert invalid.status_code == 400
    assert invalid.json()["detail"] == "Invalid dice type"
...
def test_player_can_roll_fire_bolt_spell_attack_and_scaled_damage(tmp_path, monkeypatch) -> None:
    ...
    assert damage_roll["damageComponents"][0]["die"] == "2d10"
    assert damage_roll["damageComponents"][0]["total"] == 8
...
def test_spell_damage_resolves_save_and_damage_as_one_action(tmp_path, monkeypatch) -> None:
    ...
    damage = client.post("/api/rooms/staged-spell-save-test/sheet/player-1/spells/burningHands/rolls/damage?playerKey=player-1")
    damage_resolution = client.post(f"/api/rooms/staged-spell-save-test/rolls/{damage.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    dodger = client.get("/api/rooms/staged-spell-save-test/sheet/player-2?playerKey=player-2").json()["sheet"]

    assert damage.status_code == 200
    assert "damageSaveSucceeded" not in damage.json()["roll"]
    assert damage_resolution.status_code == 200
    assert "passes DC 15 Dexterity save for half damage" in damage_resolution.json()["resolution"]["outcome"]
    assert damage_resolution.json()["resolution"]["responseRolls"][0]["label"] == "Dexterity Save"
...
def test_indomitable_prompts_on_failed_damage_save_and_can_reroll(tmp_path, monkeypatch) -> None:
    ...
    rolls = iter([1, 20])
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls) if maximum == 20 else 2)
    client = TestClient(server.app)
...
def test_resolution_prompt_can_only_be_claimed_once(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr("dnd_board.character_sheet.random.randint", lambda minimum, maximum: 2)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 1 if maximum == 20 else 2)
    client = TestClient(server.app)
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 20 if maximum == 20 else 2)
...
def test_mage_slayer_prompts_on_failed_spell_int_wis_cha_save(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 1)
    client = TestClient(server.app)
...
def test_protection_fighting_style_can_impose_disadvantage_on_attack_against_ally(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            abilityScores=AbilityScores(strength=16, dexterity=10, constitution=14, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=1, fightingStyle=FightingStyleType.PROTECTION)],
                equipment=[EquipmentItem(id="shield", name="Shield", itemType=EquipmentType.SHIELD, slot=EquipmentSlot.OFF_HAND)],
            ),
        ),
        PartyMemberConfig(
            id="player-3",
    ...
    attack_roll = client.post(
        "/api/rooms/protection-interceptor-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    prompt = client.post(
        f"/api/rooms/protection-interceptor-test/rolls/{attack_roll['id']}/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    final = client.post(
        f"/api/rooms/protection-interceptor-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=true"
    ).json()["resolution"]

    assert prompt["label"] == "Protection"
...
def test_protection_removes_critical_damage_when_disadvantage_selects_noncritical_roll(tmp_path, monkeypatch) -> None:
    ...
    attack = client.post(
        "/api/rooms/protection-critical-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    prompt = client.post(
        f"/api/rooms/protection-critical-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    resolution = client.post(
        f"/api/rooms/protection-critical-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=true"
    ).json()["resolution"]
    ally = client.get(
        "/api/rooms/protection-critical-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert resolution["roll"]["dice"] == [20, 15]
    assert resolution["roll"]["criticalHit"] is False
    assert resolution["roll"]["damageComponents"][0]["dice"] == [3]
    assert resolution["roll"]["damageComponents"][0]["total"] == 6
    assert ally["hp"]["current"] == 24


def test_critical_attack_does_not_double_independent_scheduled_damage(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    write_party_campaign(
        tmp_path,
        "critical-scheduled-damage-test",
        PartyMemberConfig(
            id="player-1",
            name="Attacker",
            maxHp=30,
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls))
    client = TestClient(server.app)
    room = server.get_or_create_room("critical-scheduled-damage-test")
    room.scheduled_effects["player-2"] = [ActiveScheduledEffect(
        id=ScheduledEffectId(40, EffectNodeId((0,))),
        sourceSheetId="player-1",
        targetSheetId="player-2",
        sourceLabel="Independent Fire",
        effect=ScheduledEffect(
            ResolutionEventType.DAMAGE_APPLIED,
            ApplyEffect(DamageEffect(DiceAmount(1, DiceType.D6), DamageType.FIRE)),
        ),
    )]

    attack = client.post(
        "/api/rooms/critical-scheduled-damage-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    resolution = client.post(
        f"/api/rooms/critical-scheduled-damage-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-2"
    ).json()["resolution"]
    target = client.get(
        "/api/rooms/critical-scheduled-damage-test/sheet/player-2?playerKey=player-2"
    ).json()["sheet"]

    assert resolution["roll"]["criticalHit"] is True
    assert resolution["roll"]["damageComponents"][0]["total"] == 5
    assert target["hp"]["current"] == 34
    assert resolution["outcome"].count("deals 1 damage") == 1


def test_interception_fighting_style_reduces_damage_to_ally(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            abilityScores=AbilityScores(strength=16, dexterity=10, constitution=14, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=1, fightingStyle=FightingStyleType.INTERCEPTION)],
                equipment=[EquipmentItem(id="shield", name="Shield", itemType=EquipmentType.SHIELD, slot=EquipmentSlot.OFF_HAND)],
            ),
        ),
        PartyMemberConfig(
    ...
    damage_roll = client.post(
        "/api/rooms/interception-interceptor-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    prompt = client.post(
        f"/api/rooms/interception-interceptor-test/rolls/{damage_roll['id']}/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    final = client.post(
        f"/api/rooms/interception-interceptor-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=true"
    ).json()["resolution"]
    ally = client.get(
        "/api/rooms/interception-interceptor-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert prompt["label"] == "Interception"
    assert prompt["ownerSheetId"] == "player-2"
    assert isinstance(prompt["effectExecutionId"], int)
    assert "Interceptor modifies the incoming damage" in final["outcome"]
    assert ally["hp"]["current"] == 35
    assert server.get_or_create_room("interception-interceptor-test").pending_effect_executions == {}


def test_multiple_reductions_stack_against_one_stable_runtime_damage_roll(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        tmp_path,
        "runtime-damage-interception-test",
        PartyMemberConfig(
            id="player-1",
            name="Source",
            maxHp=30,
    ...
    def fixed_roll(minimum: int, maximum: int) -> int:
        rolled_sides.append(maximum)
        return next(rolls)

    monkeypatch.setattr(random, "randint", fixed_roll)
    client = TestClient(server.app)
    room = server.get_or_create_room("runtime-damage-interception-test")
    room.scheduled_effects["player-3"] = [ActiveScheduledEffect(
        id=ScheduledEffectId(30, EffectNodeId((0,))),
        sourceSheetId="player-1",
        targetSheetId="player-3",
    ...
    reduction_interaction = Interaction(
    ...
    room.ongoing_effects["player-2"] = [ActiveOngoingEffect(
        id=OngoingEffectId(31, EffectNodeId((0,))),
        sourceSheetId="player-2",
        targetSheetId="player-2",
        sourceLabel="First Reduction Ward",
        effect=OngoingEffect(
            EffectDuration(EffectDurationType.MANUAL),
            interactions=[reduction_interaction],
        ),
    )]
    room.ongoing_effects["player-3"] = [ActiveOngoingEffect(
        id=OngoingEffectId(31, EffectNodeId((0,))),
        sourceSheetId="player-3",
        targetSheetId="player-3",
        sourceLabel="Second Reduction Ward",
        effect=OngoingEffect(
            EffectDuration(EffectDurationType.MANUAL),
            interactions=[reduction_interaction],
        ),
    ...
    roll = RollPayload(
        ...
        tokenId="player-1",
        roller="player-1",
        source=RollSource(SheetSectionType.ABILITIES, "conditionTrigger", "effect"),
        sourceLabel="Condition Trigger",
        ...
        total=0,
        createdAt=30,
        pendingEffect=ApplyEffect(ConditionChangeEffect(ConditionType.PRONE, ConditionOperation.ADD)),
    )
    room.pending_rolls[server.roll_queue_key(roll)] = roll

    prompt = client.post(
        "/api/rooms/runtime-damage-interception-test/rolls/condition-trigger/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    second_prompt = client.post(
        f"/api/rooms/runtime-damage-interception-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=true"
    ).json()["prompt"]
    resolution = client.post(
        f"/api/rooms/runtime-damage-interception-test/resolution-prompts/{second_prompt['id']}/respond?playerKey=player-3&use=true"
    ).json()["resolution"]
    target = client.get(
        "/api/rooms/runtime-damage-interception-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert prompt["label"] == "First Reduction Ward"
    assert second_prompt["label"] == "Second Reduction Ward"
    assert target["hp"]["current"] == 40
    assert "deals 0 damage" in resolution["outcome"]
    assert rolled_sides.count(6) == 1


def test_weapon_damage_stacks_each_eligible_interceptor_before_committing(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    interceptor_sheet = PartyMemberSheet(
        classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=1, fightingStyle=FightingStyleType.INTERCEPTION)],
        equipment=[EquipmentItem(id="shield", name="Shield", itemType=EquipmentType.SHIELD, slot=EquipmentSlot.OFF_HAND)],
    )
    write_party_campaign(
        tmp_path,
        "multiple-interceptors-test",
        PartyMemberConfig(
            id="player-1",
            name="Attacker",
            maxHp=30,
    ...
    attack = client.post(
        "/api/rooms/multiple-interceptors-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    first_prompt = client.post(
        f"/api/rooms/multiple-interceptors-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    second_response = client.post(
        f"/api/rooms/multiple-interceptors-test/resolution-prompts/{first_prompt['id']}/respond?playerKey=player-2&use=true"
    ).json()

    assert first_prompt["ownerSheetId"] == "player-2"
    assert "prompt" in second_response
    assert second_response["prompt"]["ownerSheetId"] == "player-4"
    assert second_response["prompt"]["effectExecutionId"] == first_prompt["effectExecutionId"]

    final = client.post(
        f"/api/rooms/multiple-interceptors-test/resolution-prompts/{second_response['prompt']['id']}/respond?playerKey=player-4&use=true"
    ).json()["resolution"]
    ally = client.get(
        "/api/rooms/multiple-interceptors-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert ally["hp"]["current"] == 39
    assert "First Interceptor uses Interception" in final["outcome"]
    assert "Second Interceptor uses Interception" in final["outcome"]
    assert "deals 1 damage" in final["outcome"]


def test_paused_effect_execution_does_not_overwrite_newer_participant_state(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    attack = client.post(
        "/api/rooms/stale-effect-execution-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    prompt = client.post(
        f"/api/rooms/stale-effect-execution-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-3"
    ).json()["prompt"]
    room = server.get_or_create_room("stale-effect-execution-test")
    room.hit_points["player-3"] = 35

    response = client.post(
        f"/api/rooms/stale-effect-execution-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=false"
    )
    target = client.get(
        "/api/rooms/stale-effect-execution-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert response.status_code == 409
    assert "participant state changed" in response.json()["detail"]
    assert target["hp"]["current"] == 35
    assert room.pending_effect_executions == {}


def test_damage_event_executes_bound_scheduled_and_recurring_children(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls))
    client = TestClient(server.app)
    room = server.get_or_create_room("effect-dispatch-test")
    room.scheduled_effects["player-2"] = [ActiveScheduledEffect(
        id=ScheduledEffectId(10, EffectNodeId((0,))),
        sourceSheetId="player-1",
        targetSheetId="player-3",
        sourceLabel="Scheduled Prone",
        effect=ScheduledEffect(
            ResolutionEventType.DAMAGE_APPLIED,
            SavingThrowEffect(
                SavingThrow(AbilityType.WISDOM, DifficultyClass(DifficultyClassType.FIXED, fixedValue=30)),
                onFailure=ApplyEffect(ConditionChangeEffect(ConditionType.PRONE, ConditionOperation.ADD)),
            ),
        ),
    )]
    room.ongoing_effects["player-2"] = [ActiveOngoingEffect(
        id=OngoingEffectId(11, EffectNodeId((0,))),
        sourceSheetId="player-1",
        targetSheetId="player-2",
        sourceLabel="Recurring Fire",
        effect=OngoingEffect(
            EffectDuration(EffectDurationType.UNTIL_LONG_REST),
            recurringEffects=[ScheduledEffect(
                ResolutionEventType.DAMAGE_APPLIED,
                ApplyEffect(DamageEffect(
                    CombinedAmount([
                        DiceAmount(1, DiceType.D6),
    ...
    attack = client.post(
        "/api/rooms/effect-dispatch-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    child_prompt = client.post(
        f"/api/rooms/effect-dispatch-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-2"
    ).json()["prompt"]
    assert len(room.scheduled_effects["player-2"]) == 1
    resolution = client.post(
        f"/api/rooms/effect-dispatch-test/resolution-prompts/{child_prompt['id']}/respond?playerKey=player-3&use=false"
    ).json()["resolution"]
    target = client.get(
        "/api/rooms/effect-dispatch-test/sheet/player-2?playerKey=player-2"
    ).json()["sheet"]
    bound_target = client.get(
        "/api/rooms/effect-dispatch-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    assert target["hp"]["current"] == 23
    assert child_prompt["label"] == "Indomitable"
    assert child_prompt["targetSheetId"] == "player-3"
    assert resolution["targetSheetId"] == "player-2"
    assert "prone" not in target["conditions"]
    assert "prone" in bound_target["conditions"]
    assert "deals 6 damage" in resolution["outcome"]
    assert room.scheduled_effects.get("player-2") is None


def test_dispatched_attack_rolls_against_its_bound_target(tmp_path, monkeypatch) -> None:
    longsword = AttackAction(
        id="longsword",
        name="Longsword",
        ability=AbilityType.STRENGTH,
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls))
    client = TestClient(server.app)
    room = server.get_or_create_room("dispatched-attack-test")
    room.scheduled_effects["player-2"] = [ActiveScheduledEffect(
        id=ScheduledEffectId(20, EffectNodeId((0,))),
        sourceSheetId="player-1",
        targetSheetId="player-3",
        sourceLabel="Triggered Attack",
        effect=ScheduledEffect(
            ResolutionEventType.DAMAGE_APPLIED,
            AttackRollEffect(
                AttackRoll(AttackRollType.WEAPON, AbilityType.DEXTERITY),
                onHit=ApplyEffect(ConditionChangeEffect(ConditionType.PRONE, ConditionOperation.ADD)),
            ),
        ),
    )]

    attack = client.post(
        "/api/rooms/dispatched-attack-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=longsword"
    ).json()["roll"]
    resolution = client.post(
        f"/api/rooms/dispatched-attack-test/rolls/{attack['id']}/resolve?playerKey=dm&targetSheetId=player-2"
    ).json()["resolution"]
    attack_target = client.get(
        "/api/rooms/dispatched-attack-test/sheet/player-3?playerKey=player-3"
    ).json()["sheet"]

    triggered_roll = next(roll for roll in resolution["responseRolls"] if roll["label"] == "Attack Roll")
    assert triggered_roll["dice"] == [2]
    assert triggered_roll["total"] == 4
    assert "prone" not in attack_target["conditions"]
...
def test_true_strike_roll_endpoints_use_selected_weapon_and_damage_type(tmp_path, monkeypatch) -> None:
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=30,
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=10, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(armorClass=10),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    rolls = iter([10, 5, 3])
    ...
    sheet_response = client.get("/api/rooms/true-strike-test/sheet/player-1?playerKey=player-1")
    attack = client.post("/api/rooms/true-strike-test/sheet/player-1/spells/trueStrike/rolls/weapon-attack?playerKey=player-1&equipmentInstanceId=longsword&choiceIndex=0")
    invalid_weapon = client.post("/api/rooms/true-strike-test/sheet/player-1/spells/trueStrike/rolls/weapon-attack?playerKey=player-1&equipmentInstanceId=club&choiceIndex=0")
    resolution = client.post(
        f"/api/rooms/true-strike-test/rolls/{attack.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-2"
    )

    assert sheet_response.status_code == 200
    assert [control["label"] for control in sheet_response.json()["sheet"]["spells"][0]["controls"]["actions"]] == [
        "Cast Longsword",
    ]
    assert attack.status_code == 200
    assert attack.json()["roll"]["source"]["sourceId"] == "trueStrike"
    assert attack.json()["roll"]["label"] == "True Strike"
    assert attack.json()["roll"]["boundAttackId"] == "longsword"
    assert attack.json()["roll"]["damageType"] == "slashing"
    assert [component["damageType"] for component in attack.json()["roll"]["damageComponents"]] == ["slashing", "radiant"]
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"]["current"] == 18
    assert invalid_weapon.status_code == 400


def test_shillelagh_persists_bound_weapon_and_selects_attack_modification(tmp_path, monkeypatch) -> None:
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.ROGUE, level=5)],
                spells=[shillelagh],
                attacks=[AttackAction("club", "Club", AbilityType.STRENGTH, 1, DiceType.D4, damageType=DamageType.BLUDGEONING)],
                equipment=[EquipmentItem(id="club", name="Club", definitionId=EquipmentId.CLUB, itemType=EquipmentType.WEAPON, slot=EquipmentSlot.MAIN_HAND)],
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    ...
    cast = client.post(
        "/api/rooms/shillelagh-test/sheet/player-1/spells/shillelagh/rolls/effect?playerKey=player-1&equipmentInstanceId=club"
    )
    resolved = client.post(
        f"/api/rooms/shillelagh-test/rolls/{cast.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-1"
    )
    sheet = client.get("/api/rooms/shillelagh-test/sheet/player-1?playerKey=player-1").json()["sheet"]
    option = enum_key(WeaponAttackOptionId.SPELLCASTING_ABILITY_AND_ALTERNATE_DAMAGE_TYPE)
    attack = client.post(
        f"/api/rooms/shillelagh-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=club&weaponOption={option}"
    )
    active = sheet["ongoingEffects"][0]
    removed = client.post(
        "/api/rooms/shillelagh-test/sheet/player-1/ongoing-effects/remove?playerKey=player-1",
        json={
            "resolutionSeed": active["id"]["resolutionSeed"],
            "effectNodePath": active["id"]["effectNodeId"]["path"],
        },
    )
    ...
    client.post(
        f"/api/rooms/shillelagh-test/rolls/{recast.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-1"
    )
    released = client.post(
        "/api/rooms/shillelagh-test/sheet/player-1/equipment/club/slot?playerKey=player-1&slot=carried"
    )
    client.post(
        "/api/rooms/shillelagh-test/sheet/player-1/equipment/club/slot?playerKey=player-1&slot=mainHand"
    )
    rest_cast = client.post(
        "/api/rooms/shillelagh-test/sheet/player-1/spells/shillelagh/rolls/effect?playerKey=player-1&equipmentInstanceId=club"
    )
    client.post(
        f"/api/rooms/shillelagh-test/rolls/{rest_cast.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-1"
    )
    rested = client.post(
        "/api/rooms/shillelagh-test/sheet/rest?playerKey=dm&rest=shortRest"
    )

    assert cast.status_code == 200
    assert resolved.status_code == 200
    assert sheet["attacks"][0]["activeSpellConditions"] == ["shillelagh"]
    assert len(sheet["attacks"][0]["weaponAttackOptions"]) == 4
    assert attack.status_code == 200
    assert attack.json()["roll"]["damageType"] == "force"
    assert attack.json()["roll"]["damageComponents"][0]["die"] == "1d10"
    assert attack.json()["roll"]["modifierBreakdown"][0] == {
        "source": "Wisdom",
        "value": 4,
        "description": "",
    }
    assert removed.status_code == 200
    assert removed.json()["sheet"]["ongoingEffects"] == []
    assert released.status_code == 200
    assert released.json()["sheet"]["ongoingEffects"] == []
    rested_sheet = next(sheet for sheet in rested.json()["sheets"] if sheet["id"] == "player-1")
    assert rested_sheet["ongoingEffects"] == []
...
def test_weapon_spell_requirement_failures_are_logged(tmp_path, monkeypatch) -> None:
    ...
    true_strike_blocked = client.post("/api/rooms/weapon-spell-block-test/sheet/player-1/spells/trueStrike/rolls/weapon-attack?playerKey=player-1&equipmentInstanceId=longsword&choiceIndex=1")
    shillelagh_blocked = client.post("/api/rooms/weapon-spell-block-test/sheet/player-1/spells/shillelagh/rolls/effect?playerKey=player-1&equipmentInstanceId=club")
    ...
    assert history[0]["roll"]["label"] == "Bound Weapon Attack blocked: Selected weapon is not eligible for this spell"
    assert history[1]["roll"]["label"] == "Spell Effect blocked: Selected weapon is not eligible for this spell"
...
def test_paralyzed_target_forces_dexterity_spell_save_failure(tmp_path, monkeypatch) -> None:
    burning_hands = wizard_spell_entry(SpellId.BURNING_HANDS)
    assert burning_hands is not None
    write_party_campaign(
        tmp_path,
        ...
        PartyMemberConfig(
            ...
            maxHp=30,
            abilityScores=AbilityScores(strength=8, dexterity=14, constitution=14, intelligence=16, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.WIZARD, level=3)], spells=[burning_hands]),
        ),
        PartyMemberConfig(
            id="player-2",
            name="Target",
            maxHp=30,
            abilityScores=AbilityScores(strength=10, dexterity=20, constitution=10, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=3)], conditions=[ConditionType.PARALYZED]),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    ...
    damage_response = client.post("/api/rooms/paralyzed-save-test/sheet/player-1/spells/burningHands/rolls/damage?playerKey=player-1")
    damage_roll = damage_response.json()["roll"]
    resolution = client.post(f"/api/rooms/paralyzed-save-test/rolls/{damage_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    body = resolution.json()["resolution"]

    assert damage_response.status_code == 200
    assert resolution.status_code == 200
    assert body["responseRolls"][0]["total"] == 25
    assert "fails DC 13 Dexterity save due to Paralyzed" in body["outcome"]
    assert body["targetHp"] == {"current": 24, "max": 30, "temporary": 0}
...
def test_damage_triggers_concentration_save_and_clears_sourced_conditions(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 3)
    monkeypatch.setattr("dnd_board.character_sheet.random.randint", lambda minimum, maximum: 4)
    client = TestClient(server.app)
...
def test_outgoing_attack_damage_or_spell_cast_clears_invisibility(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 10)
    client = TestClient(server.app)
...
def test_player_can_roll_tashas_hideous_laughter_effect_and_dm_can_preserve_roll(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(server_rolls) if maximum == 20 else 4)
    client = TestClient(server.app)
    ...
    assert effect_roll["damageSavingThrow"] == "wisdom"
    assert effect_roll["damageSaveDc"] == 15
    assert "pendingEffect" in effect_roll
    ...
    assert "Target fails DC 15 Wisdom save" in resolution_body["outcome"]
    assert "gains Prone; gains Incapacitated" in resolution_body["outcome"]
    ...
    assert "advantageConditions" not in damage_resolution_body["responseRolls"][0]


def test_condition_spell_resolves_save_and_effect_as_one_action(tmp_path, monkeypatch) -> None:
    tasha = wizard_spell_entry(SpellId.TASHA_S_HIDEOUS_LAUGHTER)
    assert tasha is not None
    write_party_campaign(
        tmp_path,
    ...
    effect = client.post("/api/rooms/staged-condition-save-test/sheet/player-1/spells/tashaSHideousLaughter/rolls/effect?playerKey=player-1")
    effect_resolution = client.post(f"/api/rooms/staged-condition-save-test/rolls/{effect.json()['roll']['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    ...
    assert effect.json()["roll"]["damageSavingThrow"] == "wisdom"
...
def test_calm_emotions_suppresses_existing_charmed_and_frightened_conditions(tmp_path, monkeypatch) -> None:
    ...
    assert effect_response.status_code == 200
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetConditions"] == ["charmed", "frightened", "calmEmotionsImmunity"]
    assert "gains Calm Emotions Immunity" in resolution.json()["resolution"]["outcome"]
    assert target["conditions"] == ["charmed", "frightened", "calmEmotionsImmunity"]
    assert target["suppressedConditions"] == ["charmed", "frightened"]
...
def test_spell_damage_saves_can_negate_and_damage_can_apply_conditions(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 15 if maximum == 20 else 5)
    client = TestClient(server.app)
    ...
    assert ray_roll["damageComponents"][0]["die"] == "3d8"
    assert ray_roll["damageType"] == "poison"
    assert ray_roll["pendingEffect"]["onHit"]["effects"][1]["effect"]["condition"] == "poisoned"
...
def test_slow_applies_condition_penalties_and_blocks_reactions(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 1 if maximum == 20 else 4)
    client = TestClient(server.app)
    ...
    assert effect_response.status_code == 200
    assert effect_roll["pendingEffect"]["savingThrow"]["ability"] == "wisdom"
    assert effect_roll["pendingEffect"]["onFailure"]["effect"]["condition"] == "slowed"
...
def test_incapacitated_condition_blocks_action_rolls_and_logs(tmp_path, monkeypatch) -> None:
    attack = AttackAction("club", "Club", AbilityType.STRENGTH, 1, DiceType.D4, damageType=DamageType.BLUDGEONING)
    write_party_campaign(
        tmp_path,
        "incapacitated-block-test",
        PartyMemberConfig(
            id="player-1",
            name="Target",
            maxHp=20,
            abilityScores=AbilityScores(strength=16, dexterity=10, constitution=12, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=3)], attacks=[attack], conditions=[ConditionType.INCAPACITATED]),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    client = TestClient(server.app)

    response = client.post("/api/rooms/incapacitated-block-test/sheet/player-1/rolls/attack?playerKey=player-1&attackId=club")
    blocked_entry = server.rooms["incapacitated-block-test"].roll_history[-1]

    assert response.status_code == 400
    assert response.json()["detail"] == "Incapacitated creatures cannot take Actions, Bonus Actions, or Reactions"
    assert blocked_entry.entryType == RollLogEntryType.ROLL_BLOCKED
    assert blocked_entry.roll.sourceLabel == "Club"
    assert blocked_entry.roll.label == "Attack Roll blocked: Incapacitated prevents Actions, Bonus Actions, and Reactions"


def test_incapacitated_condition_breaks_concentration(tmp_path, monkeypatch) -> None:
    bless = spell_entry(SpellId.BLESS)
    assert bless is not None
    write_party_campaign(
        tmp_path,
        "incapacitated-concentration-test",
        PartyMemberConfig(
            id="player-1",
            name="Cleric",
            maxHp=24,
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=14, intelligence=10, wisdom=16, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.CLERIC, level=3)], spells=[bless]),
        ),
        PartyMemberConfig(
            id="player-2",
            name="Ally",
    ...
    bless_effect = client.post("/api/rooms/incapacitated-concentration-test/sheet/player-1/spells/bless/rolls/effect?playerKey=player-1")
    bless_roll = bless_effect.json()["roll"]
    client.post(f"/api/rooms/incapacitated-concentration-test/rolls/{bless_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    applied = client.post("/api/rooms/incapacitated-concentration-test/sheet/player-1/conditions/incapacitated?playerKey=player-1&active=true")
    caster = client.get("/api/rooms/incapacitated-concentration-test/sheet/player-1?playerKey=player-1").json()["sheet"]
    ally = client.get("/api/rooms/incapacitated-concentration-test/sheet/player-2?playerKey=player-2").json()["sheet"]

    assert bless_effect.status_code == 200
    assert applied.status_code == 200
    assert "activeConcentration" not in caster
    assert caster["conditions"] == ["incapacitated"]
    assert ally["conditions"] == []
    assert server.rooms["incapacitated-concentration-test"].active_concentrations == {}
...
def test_prayer_of_healing_grants_short_rest_to_resolved_target(tmp_path, monkeypatch) -> None:
    ...
    assert "pendingEffect" in healing_roll
    assert resolution.status_code == 200
...
def test_heal_removes_blinded_deafened_and_poisoned_from_resolved_target(tmp_path, monkeypatch) -> None:
    heal = spell_entry(SpellId.HEAL)
    assert heal is not None
    write_party_campaign(
        tmp_path,
        "heal-condition-removal-test",
        PartyMemberConfig(
            id="player-1",
            name="Cleric",
            maxHp=60,
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=14, intelligence=10, wisdom=18, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.CLERIC, level=11)], spells=[heal]),
        ),
        PartyMemberConfig(
            id="player-2",
            name="Target",
            ...
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=14, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=11)],
                conditions=[ConditionType.BLINDED, ConditionType.DEAFENED, ConditionType.POISONED, ConditionType.PRONE],
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    client = TestClient(server.app)
    room = server.get_or_create_room("heal-condition-removal-test")
    room.hit_points["player-2"] = 1

    healing_response = client.post("/api/rooms/heal-condition-removal-test/sheet/player-1/spells/heal/rolls/healing?playerKey=player-1")
    healing_roll = healing_response.json()["roll"]
    resolution = client.post(f"/api/rooms/heal-condition-removal-test/rolls/{healing_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    target = client.get("/api/rooms/heal-condition-removal-test/sheet/player-2?playerKey=player-2").json()["sheet"]

    assert healing_response.status_code == 200
    assert "pendingEffect" in healing_roll
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"] == {"current": 60, "max": 60, "temporary": 0}
    assert "loses Blinded; loses Deafened; loses Poisoned" in resolution.json()["resolution"]["outcome"]
...
def test_aid_increases_current_and_max_hit_points_until_long_rest(tmp_path, monkeypatch) -> None:
    ...
    assert "maxHitPointIncrease" not in healing_roll
    assert healing_roll["pendingEffect"] is not None
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"] == {"current": 28, "max": 45, "temporary": 0}
    assert "increases Hit Point maximum by 5" in resolution.json()["resolution"]["outcome"]
    assert target["hp"] == {"current": 28, "max": 45, "temporary": 0}
    assert saved_data["maxHitPointIncreases"]["player-2"] == [{
        "amount": 5,
        "source": typed_json_from_value(RollSource(SheetSectionType.SPELLS, "aid", "healing-0")),
        "sourceName": "Aid",
    }]
...
def test_heroism_applies_temp_hp_and_blocks_frightened_condition(tmp_path, monkeypatch) -> None:
    ...
    assert "resists Frightened" in fear_resolution.json()["resolution"]["outcome"]
...
def test_lesser_restoration_has_separate_condition_removal_rolls(tmp_path, monkeypatch) -> None:
    ...
    assert "pendingEffect" in healing_roll
    assert resolution.status_code == 200
    assert "loses Paralyzed" in resolution.json()["resolution"]["outcome"]
...
def test_harm_failed_save_reduces_max_hit_points_until_long_rest(tmp_path, monkeypatch) -> None:
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=60,
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=10, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=11)]),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    ...
    rested_target = next(sheet for sheet in rest_response.json()["sheets"] if sheet["id"] == "player-2")
    saved_after_rest = json.loads((tmp_path / "harm-max-hp-test" / "saves" / "harm-max-hp-test.json").read_text(encoding="utf-8"))
    server.rooms.clear()
    reloaded_target = client.get("/api/rooms/harm-max-hp-test/sheet/player-2?playerKey=player-2").json()["sheet"]

    assert damage_response.status_code == 200
    assert "pendingEffect" in damage_roll
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"] == {"current": 32, "max": 32, "temporary": 0}
    assert "reduces Hit Point maximum by 28" in resolution.json()["resolution"]["outcome"]
    assert target["hp"] == {"current": 32, "max": 32, "temporary": 0}
    assert saved_data["maxHitPointReductions"]["player-2"] == [
        {
            "amount": 28,
            "source": typed_json_from_value(RollSource(SheetSectionType.SPELLS, "harm", "damage-0")),
            "sourceName": "Harm",
            "reset": "longRest",
        }
    ]
...
def test_harm_successful_save_does_not_reduce_max_hit_points(tmp_path, monkeypatch) -> None:
    harm = spell_entry(SpellId.HARM)
    assert harm is not None
    write_party_campaign(
        tmp_path,
    ...
    damage_response = client.post("/api/rooms/harm-save-test/sheet/player-1/spells/harm/rolls/damage?playerKey=player-1")
    damage_roll = damage_response.json()["roll"]
    resolution = client.post(f"/api/rooms/harm-save-test/rolls/{damage_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    target = client.get("/api/rooms/harm-save-test/sheet/player-2?playerKey=player-2").json()["sheet"]

    assert damage_response.status_code == 200
    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"] == {"current": 46, "max": 60, "temporary": 0}
    assert "Hit Point maximum is reduced" not in resolution.json()["resolution"]["outcome"]
    assert target["hp"] == {"current": 46, "max": 60, "temporary": 0}
    assert server.get_or_create_room("harm-save-test").max_hit_point_reductions == {}
...
def test_shatter_rolls_disadvantage_for_construct_targets(tmp_path, monkeypatch) -> None:
    ...
    rolls = iter([18, 2])
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls) if maximum == 20 else 4)
    client = TestClient(server.app)
...
def test_damage_save_can_force_failure_for_creature_type(tmp_path, monkeypatch) -> None:
    blight = wizard_spell_entry(SpellId.BLIGHT)
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=30,
            abilityScores=AbilityScores(strength=8, dexterity=14, constitution=14, intelligence=16, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.WIZARD, level=7)], spells=[blight]),
        ),
        PartyMemberConfig(
            id="player-2",
            name="Plant",
            maxHp=80,
            abilityScores=AbilityScores(strength=10, dexterity=10, constitution=20, intelligence=10, wisdom=10, charisma=10),
            sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.CREATURE, level=1)], creatureTypes=[CreatureType.PLANT]),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 20 if maximum == 20 else 4)
    client = TestClient(server.app)

    damage_response = client.post("/api/rooms/blight-plant-test/sheet/player-1/spells/blight/rolls/damage?playerKey=player-1")
    damage_roll = damage_response.json()["roll"]
    resolution = client.post(f"/api/rooms/blight-plant-test/rolls/{damage_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    body = resolution.json()["resolution"]

    assert damage_response.status_code == 200
    assert damage_roll["damageSaveForcedFailureCreatureTypes"] == ["plant"]
    assert resolution.status_code == 200
    assert body["responseRolls"][0]["dice"] == [20]
    assert "fails DC 14 Constitution save due to Plant" in body["outcome"]
    assert body["targetHp"]["current"] == 48
...
def test_vampiric_touch_heals_source_for_half_final_damage(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 15 if maximum == 20 else 4)
    client = TestClient(server.app)
    ...
    assert damage_response.status_code == 200
    assert "sourceHealing" not in damage_roll
    assert damage_roll["pendingEffect"] is not None
    assert damage_roll["label"] == "Vampiric Touch"
    assert damage_roll["die"] == "d20"
    assert damage_roll["damageComponents"][0]["die"] == "3d6"
    assert resolution.status_code == 200
...
def test_combined_vampiric_touch_attack_respects_uncanny_dodge_before_source_healing(tmp_path, monkeypatch) -> None:
    vampiric_touch = wizard_spell_entry(SpellId.VAMPIRIC_TOUCH)
    assert vampiric_touch is not None
    write_party_campaign(
        tmp_path,
        "vampiric-touch-uncanny-dodge-test",
        PartyMemberConfig(
            id="player-1",
            name="Wizard",
            maxHp=30,
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 15 if maximum == 20 else 4)
    client = TestClient(server.app)
    room = server.get_or_create_room("vampiric-touch-uncanny-dodge-test")
    room.hit_points["player-1"] = 10

    combined_roll = client.post(
        "/api/rooms/vampiric-touch-uncanny-dodge-test/sheet/player-1/spells/vampiricTouch/rolls/damage?playerKey=player-1"
    ).json()["roll"]
    prompt_response = client.post(
        f"/api/rooms/vampiric-touch-uncanny-dodge-test/rolls/{combined_roll['id']}/resolve?playerKey=dm&targetSheetId=player-2"
    )
    prompt = prompt_response.json()["prompt"]
    final_response = client.post(
        f"/api/rooms/vampiric-touch-uncanny-dodge-test/resolution-prompts/{prompt['id']}/respond?playerKey=player-2&use=true"
    )
    caster = client.get(
        "/api/rooms/vampiric-touch-uncanny-dodge-test/sheet/player-1?playerKey=player-1"
    ).json()["sheet"]
    target = client.get(
        "/api/rooms/vampiric-touch-uncanny-dodge-test/sheet/player-2?playerKey=player-2"
    ).json()["sheet"]

    assert prompt["label"] == "Uncanny Dodge"
    assert final_response.status_code == 200
    assert target["hp"]["current"] == 24
    assert caster["hp"]["current"] == 13
    assert "halves the incoming damage" in final_response.json()["resolution"]["outcome"]
...
def test_player_can_roll_ability_check_and_saving_throw(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    client = TestClient(server.app)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 11)
...
def test_player_can_update_owned_sheet_resource(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=36,
            abilityScores=AbilityScores(strength=16, dexterity=14, constitution=15, intelligence=10, wisdom=12, charisma=8),
            baseAbilityScores=AbilityScores(strength=14, dexterity=14, constitution=15, intelligence=10, wisdom=12, charisma=8),
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=4, fightingStyles=[FightingStyleType.DEFENSE])],
                progressionGrants=[
                    *hit_point_grants(ClassType.FIGHTER, 8, 8, 8),
                    *ability_score_grants(ClassType.FIGHTER, 4, AbilityType.STRENGTH, 2),
                ],
            ),
        ),
    )
...
def test_player_can_update_exhaustion_level_and_long_rest_reduces_it(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        tmp_path,
        "exhaustion-level-test",
        PartyMemberConfig(
            id="player-1",
            name="Tired Fighter",
            maxHp=40,
    ...
    denied = client.post("/api/rooms/exhaustion-level-test/sheet/player-1/exhaustion?playerKey=player-2&level=4")
    applied = client.post("/api/rooms/exhaustion-level-test/sheet/player-1/exhaustion?playerKey=player-1&level=4")
    dead = client.post("/api/rooms/exhaustion-level-test/sheet/player-1/exhaustion?playerKey=player-1&level=6")
    revived = client.post("/api/rooms/exhaustion-level-test/sheet/player-1/conditions/dead?playerKey=player-1&active=false")
    saved_data = json.loads((tmp_path / "exhaustion-level-test" / "saves" / "exhaustion-level-test.json").read_text(encoding="utf-8"))
    rest_response = client.post("/api/rooms/exhaustion-level-test/sheet/rest?playerKey=dm&rest=long")
    rested = next(sheet for sheet in rest_response.json()["sheets"] if sheet["id"] == "player-1")

    assert denied.status_code == 403
    assert applied.status_code == 200
    assert applied.json()["sheet"]["conditions"] == ["exhaustion"]
    assert applied.json()["sheet"]["exhaustionLevel"] == 4
    assert applied.json()["sheet"]["hp"] == {"current": 40, "max": 40, "temporary": 0}
    assert applied.json()["sheet"]["speed"] == 10
    assert dead.status_code == 200
    assert dead.json()["sheet"]["conditions"] == ["dead", "exhaustion"]
    assert revived.status_code == 200
    assert revived.json()["sheet"]["conditions"] == ["exhaustion"]
    assert saved_data["exhaustionLevels"] == {"player-1": 6}
    assert rest_response.status_code == 200
    assert rested["conditions"] == ["exhaustion"]
    assert rested["exhaustionLevel"] == 5
    assert rested["hp"] == {"current": 40, "max": 40, "temporary": 0}
    assert rested["speed"] == 5
...
def test_fighter_ability_score_improvement_updates_hp_when_constitution_modifier_changes(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=36,
            abilityScores=AbilityScores(strength=16, dexterity=14, constitution=15, intelligence=10, wisdom=12, charisma=8),
            baseAbilityScores=AbilityScores(strength=16, dexterity=14, constitution=15, intelligence=10, wisdom=12, charisma=8),
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=4)],
                progressionGrants=hit_point_grants(ClassType.FIGHTER, 8, 8, 8),
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                ...
                progressionGrants=feat_grants(ClassType.FIGHTER, 4, GeneralFeatType.ACTOR),
                feats=[
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                ...
                progressionGrants=hit_point_grants(ClassType.FIGHTER, 8),
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
...
def test_level_down_prunes_unavailable_fighter_choices_and_hp(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                ...
                progressionGrants=hit_point_grants(ClassType.FIGHTER, 8, 8),
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    ...
    releveled = client.post("/api/rooms/level-down-test/sheet/player-1/level?playerKey=dm&delta=1")
    releveled_sheet = releveled.json()["sheet"]

    assert releveled.status_code == 200
    assert "subclass" not in releveled_sheet["classes"][0]
    assert "fighterSubclass" in {
        choice["id"] for choice in releveled_sheet["pendingChoices"]
    }
...
def test_level_down_prunes_fighter_ability_score_improvements(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=4)],
                progressionGrants=ability_score_grants(
                    ClassType.FIGHTER,
                    4,
                    AbilityType.STRENGTH,
                    2,
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                ...
                progressionGrants=hit_point_grants(ClassType.WIZARD, 4, 4),
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
...
def test_player_can_choose_eldritch_knight_spells(tmp_path, monkeypatch) -> None:
    ...
    assert spells["shield"]["source"] == "eldritchKnight"
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            sheet=PartyMemberSheet(
                ...
                progressionGrants=hit_point_grants(ClassType.ROGUE, 7, 7),
            ),
        ),
    )
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
...
def test_rogue_levels_into_soulknife_and_uses_homing_strikes(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 4)
    client = TestClient(server.app)
...
def test_level_down_prunes_eldritch_knight_spells(tmp_path, monkeypatch) -> None:
    ...
    write_party_campaign(
        ...
        PartyMemberConfig(
            ...
            maxHp=58,
            abilityScores=AbilityScores(strength=16, dexterity=14, constitution=15, intelligence=14, wisdom=12, charisma=8),
            baseAbilityScores=AbilityScores(strength=14, dexterity=14, constitution=15, intelligence=14, wisdom=12, charisma=8),
            sheet=PartyMemberSheet(
                ...
                progressionGrants=[
                    *hit_point_grants(ClassType.FIGHTER, 8, 8, 8, 8, 8, 8),
                    *ability_score_grants(ClassType.FIGHTER, 4, AbilityType.STRENGTH, 2),
                ],
                spells=[
...
def test_serialized_tracker_recovery_rules_override_definition_defaults() -> None:
    ...
    recovered = reset_sheet_resources(room, sheet, RestType.SHORT_REST)
...
def test_tactical_mind_roll_consumes_second_wind_and_has_own_pending_slot(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    client = TestClient(server.app)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 7)
...
def test_superior_technique_roll_consumes_superiority_die(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    client = TestClient(server.app)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 5)
...
def test_cavalier_warding_maneuver_roll_consumes_resource(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 6)
    client = TestClient(server.app)
...
def test_damage_roll_resolution_reduces_target_hp(monkeypatch) -> None:
    client = TestClient(server.app)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 8)
...
def test_lethal_damage_marks_target_dead(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        tmp_path,
        "sheet-death-test",
        PartyMemberConfig(
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 8)
    client = TestClient(server.app)
    room = server.get_or_create_room("sheet-death-test")
    room.hit_points["player-2"] = 5

    damage = client.post("/api/rooms/sheet-death-test/sheet/player-1/rolls/damage?playerKey=player-1&attackId=club").json()["roll"]
    resolution = client.post(f"/api/rooms/sheet-death-test/rolls/{damage['id']}/resolve?playerKey=dm&targetSheetId=player-2")
    target_sheet = client.get("/api/rooms/sheet-death-test/sheet/player-2?playerKey=player-1").json()["sheet"]

    assert resolution.status_code == 200
    assert resolution.json()["resolution"]["targetHp"]["current"] == 0
    assert "dead" in resolution.json()["resolution"]["targetConditions"]
    assert "gains Dead" in resolution.json()["resolution"]["outcome"]
    assert "dead" in target_sheet["conditions"]
    assert target_sheet["speed"] == 0
...
def test_damage_roll_resolution_applies_resistance_vulnerability_and_immunity(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server, "CAMPAIGN_DIR", tmp_path)
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: 9)
    client = TestClient(server.app)
            ...
            sheet=PartyMemberSheet(
                classes=[
                    CharacterClassLevel(
                        ...
                        classOptions=[ClassOptionSelection(ClassOptionKind.MANEUVER, BattleMasterManeuverType.RALLY)],
                    )
                ],
            ),
        ),
...
def test_condition_roll_resolution_applies_condition_on_failed_save(tmp_path, monkeypatch) -> None:
            ...
            sheet=PartyMemberSheet(
                classes=[
                    CharacterClassLevel(
                        ...
                        classOptions=[ClassOptionSelection(ClassOptionKind.ARCANE_SHOT, ArcaneShotType.SHADOW_ARROW)],
                    )
                ]
            ),
        ),
    ...
    assert resolution.status_code == 200
    assert "conditionEffects" not in roll
...
def test_resolution_records_and_resolves_each_reachable_saving_throw_node(tmp_path, monkeypatch) -> None:
    write_party_campaign(
        tmp_path,
        "nested-save-node-test",
        PartyMemberConfig(
            id="player-1",
            name="Source",
            maxHp=30,
    ...
    monkeypatch.setattr(random, "randint", lambda minimum, maximum: next(rolls))
    client = TestClient(server.app)
    room = server.get_or_create_room("nested-save-node-test")
    pending = RollPayload(
        id="nested-save-roll",
        sheetId="player-1",
        tokenId="player-1",
        roller="player-1",
        source=RollSource(SheetSectionType.ABILITIES, "nestedSave", "effect"),
        sourceLabel="Nested Save",
        resolution=RollResolutionMode.NONE,
        label="Nested Save",
        iconUrl=None,
        dice=[],
        ...
        pendingEffect=SequenceEffect([
            SavingThrowEffect(
                SavingThrow(AbilityType.DEXTERITY, DifficultyClass(DifficultyClassType.FIXED, fixedValue=20)),
                onFailure=ApplyEffect(ConditionChangeEffect(ConditionType.PRONE, ConditionOperation.ADD)),
            ),
            SavingThrowEffect(
                SavingThrow(AbilityType.CONSTITUTION, DifficultyClass(DifficultyClassType.FIXED, fixedValue=20)),
                onFailure=ApplyEffect(ConditionChangeEffect(ConditionType.POISONED, ConditionOperation.ADD)),
            ),
        ]),
    )
    room.pending_rolls[server.roll_queue_key(pending)] = pending

    response = client.post(
        "/api/rooms/nested-save-node-test/rolls/nested-save-roll/resolve?playerKey=dm&targetSheetId=player-2"
    )
    resolution = response.json()["resolution"]

    assert [roll["label"] for roll in resolution["responseRolls"]] == ["Dexterity Save", "Constitution Save"]
    assert resolution["targetConditions"] == ["prone", "poisoned"]
    assert [entry["effectNodeId"]["path"] for entry in resolution["roll"]["effectInputs"]["rolls"]] == [[0], [1]]
...
def test_condition_roll_resolution_survives_sheet_poll_without_manifest(tmp_path, monkeypatch) -> None:
    ...
    roll = RollPayload(
        ...
        total=1,
        createdAt=1,
        damageSavingThrow=AbilityType.STRENGTH,
        damageSaveDc=30,
        damageSaveOutcome=SpellSaveOutcome.NEGATES,
        pendingEffect=SavingThrowEffect(
            SavingThrow(AbilityType.STRENGTH, DifficultyClass(DifficultyClassType.FIXED, fixedValue=30)),
            onFailure=ApplyEffect(ConditionChangeEffect(ConditionType.PRONE, ConditionOperation.ADD)),
        ),
    )
    room.pending_rolls[server.roll_queue_key(roll)] = roll
...
def test_protection_from_poison_grants_advantage_against_poisoned_condition(tmp_path, monkeypatch) -> None:
    ...
    poison_roll = RollPayload(
        ...
        total=0,
        createdAt=1,
        damageSavingThrow=AbilityType.CONSTITUTION,
        damageSaveDc=15,
        damageSaveOutcome=SpellSaveOutcome.NEGATES,
        pendingEffect=SavingThrowEffect(
            SavingThrow(AbilityType.CONSTITUTION, DifficultyClass(DifficultyClassType.FIXED, fixedValue=15)),
            onFailure=ApplyEffect(ConditionChangeEffect(ConditionType.POISONED, ConditionOperation.ADD)),
        ),
    )
...
def test_pushing_attack_triggers_visible_strength_save_without_condition(tmp_path, monkeypatch) -> None:
            ...
            sheet=PartyMemberSheet(
                classes=[
                    CharacterClassLevel(
                        ...
                        level=3,
                        subclass=FighterSubclassType.BATTLE_MASTER,
                        classOptions=[ClassOptionSelection(ClassOptionKind.MANEUVER, BattleMasterManeuverType.PUSHING_ATTACK)],
                    )
                ],
            ),
        ),
    ...
    assert resolution.status_code == 200
    assert "conditionEffects" not in roll
    assert roll["damageSavingThrow"] == "strength"
...
def test_grappling_strike_resolves_contested_check_and_applies_grappled(tmp_path, monkeypatch) -> None:
            ...
            sheet=PartyMemberSheet(
                ...
                classes=[
                    CharacterClassLevel(
                        ...
                        level=3,
                        subclass=FighterSubclassType.BATTLE_MASTER,
                        classOptions=[ClassOptionSelection(ClassOptionKind.MANEUVER, BattleMasterManeuverType.GRAPPLING_STRIKE)],
                    )
                ],
            ),
        ),
    ...
    assert resolution.status_code == 200
    assert "conditionEffects" not in roll
    assert roll["pendingEffect"]["contest"]["sourceCheck"]["skill"] == "athletics"
...
def test_grappling_strike_does_not_apply_grappled_when_target_wins_contest(tmp_path, monkeypatch) -> None:
    ...
    assert resolution.status_code == 200
    assert "Grappler fails" in resolution.json()["resolution"]["outcome"]
    assert resolution.json()["resolution"]["targetConditions"] == []
...
def test_server_state_helpers_cover_rest_equipment_and_room_edges(tmp_path, monkeypatch) -> None:
    ...
    reset_character_for_rest(room, sheet, RestType.SHORT_REST, server.character_state_persistence())
    ...
    reset_character_for_rest(room, sheet, RestType.LONG_REST, server.character_state_persistence())
    ...
    assert reset_applies_to_rest(RestType.NONE, RestType.LONG_REST) is False
    ...
    set_equipment_slot(room, sheet, "new-armor", EquipmentSlot.ARMOR)
    set_equipment_slot(room, sheet, "greatsword", EquipmentSlot.TWO_HANDS)
    set_equipment_slot(room, sheet, "dagger", EquipmentSlot.MAIN_HAND)
    ...
    append_roll_log_entry(
        room,
        RollLogEntry("last", RollLogEntryType.ROLL_CREATED, 999, history_roll),
        server.ROLL_HISTORY_LIMIT,
    )
...
def test_member_progression_helper_error_and_pruning_paths(monkeypatch) -> None:
    ...
    assert unavailable.value.detail == "Unsupported progression rule"
    ...
    for values, detail in [
        ...
        (["luck"], "Invalid ability score or no selected score can be increased"),
    ...
    assert max_error.value.detail == "Invalid ability score or no selected score can be increased"
    ...
    with pytest.raises(Exception) as duplicate_feat:
        apply_member_progression_rule(
            PartyMemberConfig(id="player-1", name="Feat", sheet=PartyMemberSheet(classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=8)], feats=feat_member.sheet.feats)),
            ProgressionChoiceId.FIGHTER_ABILITY_SCORE_IMPROVEMENT,
            ["feat:actor"],
        )
    assert duplicate_feat.value.detail == "Feat is already selected"

    pruning_member = PartyMemberConfig(
        id="player-1",
        name="Pruning Fighter",
        abilityScores=AbilityScores(12, 10, 10, 10, 10, 10),
        baseAbilityScores=AbilityScores(10, 10, 10, 10, 10, 10),
        sheet=PartyMemberSheet(
            classes=[CharacterClassLevel(name=ClassType.FIGHTER, level=6)],
            feats=[general_feat_feature("actor")],
            progressionGrants=[
                *ability_score_grants(ClassType.FIGHTER, 4, AbilityType.STRENGTH, 2),
                *feat_grants(ClassType.FIGHTER, 6, GeneralFeatType.ACTOR),
            ],
        ),
    )
    set_member_class_levels(
        pruning_member,
        [CharacterClassLevel(name=ClassType.FIGHTER, level=4)],
    )
    assert pruning_member.abilityScores == AbilityScores(12, 10, 10, 10, 10, 10)
    assert pruning_member.sheet.feats is None
...
def test_saved_state_and_asset_parsing_fallbacks(tmp_path, monkeypatch) -> None:
    ...
    assert load_room_save(save_dir / "missing.json") is None
    assert load_room_save(bad_path) is None

    resources_path = save_dir / "resources.json"
    resources_path.write_text(json.dumps({"tokens": [], "resources": {" Player 1! ": {"Second Wind": "2", "bad": "x"}, "bad": []}}), encoding="utf-8")
    loaded = load_room_save(resources_path)
    assert loaded is not None
    assert loaded.resource_uses == {"player1": {"secondwind": 2}}
